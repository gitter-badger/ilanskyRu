...
@section('title')
Настройка доступа группы "{{{$role->name}}}"
@stop
@section('content')
<div class="row">
    <div class="col-lg-12">
        {{ Form::open(array('route' => array('admin-roles-access', $role->id))) }}
        <table class="table">
            <thead>
            <tr>
                <th>ID</th>
        ...
        <tr>
            ...
            <td>{{ Form::checkbox("resource_id[]", $resource->id, $role->resources->contains($resource->id)) }}</td>
        </tr>
        @endforeach
            </tbody>
        </table>
